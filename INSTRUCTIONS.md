...
- Follow the project's architecture layer boundaries strictly: presentation → logic → data
...
- Any reusable logic, utility, constant, extension, or helper used in 2+ places goes in `core/widgets`
...
- Write tests for domain (if we have domain) and data layer logic
...
## 3) Domain Layer Purity(if we Have)
...
- Domain layer: return `ApiResult<T>` from use cases and repositories(if we Have)
...
- Use `BlocBuilder`/`BlocConsumer` on the smallest widget that needs the state — never at the top of the tree
